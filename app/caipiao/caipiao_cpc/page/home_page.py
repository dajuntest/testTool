...
class CpCHomePage(Page):
    ...
    cpchp_login_password_input = PageElement(xpath="//input[@placeholder='密码']")
    cpchp_verify_number_button = PageElement(xpath="//div[@class='code-boxs']//img")
    cpchp_verify_number_input = PageElement(xpath="//input[@placeholder='验证码']")
    cpchp_login_button = PageElement(xpath="//button[contains(.,'登录')]")
    cpchp_register_button = PageElement(xpath="//button[contains(.,'立即注册')]")

    popup = PageElement(xpath="//button[@class='btn_ok bg-col'][contains(.,'确定')]") # todo 归到其他类中去
